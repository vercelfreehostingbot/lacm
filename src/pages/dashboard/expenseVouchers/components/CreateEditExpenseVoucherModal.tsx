...
import type { ExpenseVoucher } from "../../../../redux/features/expenseVouchers/types";
import { useCreateExpenseVoucherMutation, useUpdateExpenseVoucherMutation } from "../../../../redux/features/expenseVouchers/expenseVouchersApi";
...
interface CreateEditExpenseVoucherModalProps {
  t: TFunction;
  isOpen: boolean;
  mode: "create" | "edit";
...
export function CreateEditExpenseVoucherModal({
  t,
  isOpen,
  mode,
  voucher,
  onClose,
  onSaved,
}: CreateEditExpenseVoucherModalProps) {
  const [createVoucher, { isLoading: isCreating }] = useCreateExpenseVoucherMutation();
  const [updateVoucher, { isLoading: isUpdating }] = useUpdateExpenseVoucherMutation();
  ...
  const [expenseHead, setExpenseHead] = useState(voucher?.expenseHead ?? "");
  ...
  const resetFields = () => {
    ...
    setExpenseHead(voucher?.expenseHead ?? "");
  ...
  const submitForm = async () => {
    ...
    if (!date || !amount.trim() || !expenseHead.trim() || !description.trim() || !reference.trim()) {
  ...
  return (
    <Modal onClose={handleClose} closeOnBackdropClick={false}>
      <div className="max-h-[88vh] w-140 max-w-[92vw] overflow-y-auto rounded-2xl bg-surface shadow-2xl">
        <div className="flex items-center justify-between border-b border-surface-border px-6 py-5">
          ...
          <button
            onClick={handleClose}
            className="cursor-pointer flex h-7.5 w-7.5 items-center justify-center rounded-lg bg-chip-bg text-text-secondary transition hover:bg-chip-bg-hover"
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex flex-col gap-4 px-6 py-5.5">
          ...
          <div className="flex gap-3.5">
            ...
            <div className="flex-1">
              <FormLabel required>{t("form.expenseHead")}</FormLabel>
              <input
                value={expenseHead}
                onChange={(e) => setExpenseHead(e.target.value)}
                placeholder={t("form.expenseHeadPlaceholder")}
                disabled={isSaving}
                className="w-full cursor-text rounded-[9px] border-[1.5px] border-surface-border px-3 py-2.5 text-[13.5px] text-text-primary outline-none transition disabled:cursor-not-allowed disabled:opacity-60"
              />
            </div>
