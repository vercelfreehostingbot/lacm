...
import { CreateEditIncomeVoucherModal } from "./components/CreateEditIncomeVoucherModal";
import {
...
import type { IncomeVoucher, VoucherStatus } from "../../../redux/features/incomeVouchers/types";
...
export default function IncomeVouchersPage() {
  const { t, i18n } = useTranslation("incomeVouchers");
  ...
  const { data: stats, isLoading: isStatsLoading } = useGetIncomeVoucherStatsQuery();
  ...
  const [editingVoucher, setEditingVoucher] = useState<IncomeVoucher | null>(null);

  const openCreate = () => {
    setEditingVoucher(null);
    setFormOpen(true);
  };
  const openEdit = (voucher: IncomeVoucher) => {
    setEditingVoucher(voucher);
    setFormOpen(true);
  };

  // ---- View modal ----
  const [viewingVoucher, setViewingVoucher] = useState<IncomeVoucher | null>(null);

  // ---- Delete ----
  const [deletingVoucher, setDeletingVoucher] = useState<IncomeVoucher | null>(null);
  const [deleteVoucher, { isLoading: isDeleting }] = useDeleteIncomeVoucherMutation();
  ...
  // ---- Approve / Reject ----
  const [approveVoucher, { isLoading: isApproving }] = useApproveIncomeVoucherMutation();
  const [rejectVoucher, { isLoading: isRejecting }] = useRejectIncomeVoucherMutation();

  const handleApprove = async (voucher: IncomeVoucher) => {
    try {
  ...
  const handleReject = async (voucher: IncomeVoucher) => {
    try {
  ...
  // ---- Summary PDF ----
  const [triggerSummaryPdf] = useLazyDownloadIncomeVoucherSummaryPdfQuery();
  ...
  const handleDownloadSummaryPdf = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = "income-vouchers-summary.pdf";
  ...
  // ---- PDF download ----
  const [triggerDownloadPdf] = useLazyDownloadIncomeVoucherPdfQuery();
  const [downloadingId, setDownloadingId] = useState<string | null>(null);
  const [printingId, setPrintingId] = useState<string | null>(null);

  const handleDownloadPdf = async (voucher: IncomeVoucher) => {
  ...
  const handlePrintPdf = async (voucher: IncomeVoucher) => {
  ...
  const columns: DataTableColumn<IncomeVoucher>[] = [
    {
      key: "sl",
      header: t("table.sl"),
      render: (_row, index) => (
      ...
      {viewingVoucher && (
        <DetailModal
          ...
          fields={[
            ...
            { label: t("table.source"), value: viewingVoucher.incomeSource },
